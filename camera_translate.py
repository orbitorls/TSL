...
class ThaiSignTranslator:
    ...
    def _get_cached_font(self, font_size, primary_font="tahoma", fallback_font="phagspa"):
        """Get font from cache or load it if not available."""
        cache_key = (font_size, primary_font)
        if cache_key in self._font_cache:
            return self._font_cache[cache_key]

        try:
            font = ImageFont.truetype(f"C:/Windows/Fonts/{primary_font}.ttf", font_size)
        except Exception:
            try:
                font = ImageFont.truetype(f"C:/Windows/Fonts/{fallback_font}.ttf", font_size)
            except Exception:
                # Try generic font list as last resort for specific sizes
                if primary_font == "generic":
                    font_paths = [
    ...
    def _draw_thai_text(self, frame, text, pos, font_size=60, color=(255, 255, 255), bg_color=None):
        ...
        font = self._get_cached_font(font_size, primary_font="generic")
