...
class ThaiSignTranslator:
    ...
    def _get_cached_font(self, font_size, font_paths=None):
        """Get a cached font to avoid redundant disk I/O during real-time rendering."""
        ...
        cache_key = f"{font_size}_{'-'.join(font_paths)}"
        if cache_key not in self._font_cache:
            ...
            for font_path in font_paths:
                try:
                    ...
                    continue
            self._font_cache[cache_key] = font if font is not None else ImageFont.load_default(size=font_size)
