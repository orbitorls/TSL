...
def extract_features_from_landmark_df(lm_df: Any, feature_level: str = "basic") -> np.ndarray:
    ...
    if len(available_cols) > 0:
        means = lm_df[available_cols].mean(numeric_only=True).fillna(0.0)
        features = means.reindex(expected_cols, fill_value=0.0).to_numpy(dtype=np.float32)
    else:
        features = np.zeros(len(expected_cols), dtype=np.float32)

    return features[: get_feature_dim(feature_level)]
