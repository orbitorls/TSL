...
def extract_features_from_landmark_df(lm_df: Any, feature_level: str = "basic") -> np.ndarray:
    ...
    # Generate the target column list once
    target_cols = _build_column_list(feature_level)

    # Identify which columns actually exist in the DataFrame
    # Using df.columns.intersection is fast but might reorder, so we filter instead
    available_cols = [col for col in target_cols if col in lm_df.columns]
    ...
    features = [means.get(col, 0.0) for col in target_cols]
...
def extract_sequence_from_landmark_df(
    ...
    available_cols = [col for col in col_list if col in lm_df.columns]
    if available_cols:
        indices = [col_list.index(col) for col in available_cols]
        seq[:, indices] = lm_df[available_cols].fillna(0.0).to_numpy(dtype=np.float32)
