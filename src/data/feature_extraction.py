...
def _build_column_list(feature_level: str = "basic") -> list[str]:
    """Build the expected list of column names for a given feature level.
    Results are cached in _FEATURE_COLUMN_CACHE to avoid redundant string formatting.
    """
    ...
    cols = cols[: get_feature_dim(feature_level)]
...
def extract_features_from_landmark_df(lm_df: Any, feature_level: str = "basic") -> np.ndarray:
    ...
    # ⚡ Performance Optimization: Vectorized mean calculation & Cached columns
    # Eliminates redundant string formatting and replaces slow iterative safe_mean calls
    expected_cols = _build_column_list(feature_level)
    avail_cols = [c for c in expected_cols if c in lm_df.columns]
