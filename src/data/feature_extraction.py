...
def extract_features_from_landmark_df(lm_df: Any, feature_level: str = "basic") -> np.ndarray:
    ...
    if len(available_cols) > 0:
        # Vectorized mean computation using Pandas
        col_indices = pd.Index(col_list).get_indexer(available_cols)
        features[col_indices] = lm_df[available_cols].mean(numeric_only=True).fillna(0.0).to_numpy(dtype=np.float32)
...
def extract_sequence_from_landmark_df(
    ...
    col_list = _build_column_list(feature_level)
    seq = np.zeros((n_frames, len(col_list)), dtype=np.float32)
    ...
    # Note: seq could have slightly more columns than feature_dim if feature_level='full' truncates face,
    # but seq size must exactly match feature_dim, so slice before sampling.
    seq = seq[:, :feature_dim]
