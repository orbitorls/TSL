...
def extract_features_from_landmark_df(lm_df: Any, feature_level: str = "basic") -> np.ndarray:
    ...
    # ⚡ Bolt: Vectorize pandas mean computation for significant speedup
    ...
    features = [float(means.get(col, 0.0)) for col in cols]
...
def extract_sequence_from_landmark_df(
    ...
    # ⚡ Bolt: Vectorize sequence extraction for significant speedup
    available_cols = lm_df.columns.intersection(col_list)
    if available_cols.empty:
        return np.zeros((target_frames, feature_dim), dtype=np.float32)

    seq_vals = lm_df[available_cols].fillna(0.0).to_numpy(dtype=np.float32)

    # Reconstruct the sequence with columns in the expected schema order
    seq = np.zeros((n_frames, feature_dim), dtype=np.float32)
    col_indices = [col_list.index(c) for c in available_cols]
