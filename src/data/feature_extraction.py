...
def extract_features_from_landmark_df(lm_df: Any, feature_level: str = "basic") -> np.ndarray:
    ...
    # Vectorized computation for significant performance gain
    cols = _build_column_list(feature_level)
    available_cols = lm_df.columns.intersection(cols)
    means = lm_df[available_cols].mean().fillna(0.0).to_dict() if not available_cols.empty else {}
    features = [means.get(c, 0.0) for c in cols]
