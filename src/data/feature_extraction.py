...
# Module-level cache to avoid redundant list formatting on every function call.
...
def extract_features_from_landmark_df(lm_df: Any, feature_level: str = "basic") -> np.ndarray:
    ...
    # Use vectorized operations to compute means significantly faster than iterative safe_mean
    if hasattr(lm_df, "columns"):
        ...
        # Fallback if lm_df is not a pandas DataFrame but still dictionary-like
        means = {}
