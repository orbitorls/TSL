...
def extract_features_from_landmark_df(lm_df: Any, feature_level: str = "basic") -> np.ndarray:
    ...
    # Vectorized computation of means provides ~20x speedup
    means = lm_df[available_cols].mean().fillna(0.0).to_dict() if len(available_cols) > 0 else {}
...
def extract_sequence_from_landmark_df(
    ...
    # Vectorized bulk assignment provides ~10x speedup
    if not available_cols.empty:
        col_to_idx = {c: i for i, c in enumerate(col_list)}
        indices = [col_to_idx[c] for c in available_cols]
