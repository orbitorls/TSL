...
def extract_features_from_landmark_df(lm_df: Any, feature_level: str = "basic") -> np.ndarray:
    ...
    # Fast path: Vectorized extraction using pandas instead of iterative loops
    present_cols = lm_df.columns.intersection(col_list)
    means_dict = lm_df[present_cols].mean(numeric_only=True).fillna(0.0).to_dict()
    features = [means_dict.get(col, 0.0) for col in col_list]
...
def extract_sequence_from_landmark_df(
    ...
    # Fast path: vectorized block assignment
    present_cols = lm_df.columns.intersection(col_list)
    if len(present_cols) > 0:
        indices = [col_list.index(col) for col in present_cols]
        seq[:, indices] = lm_df[present_cols].fillna(0.0).to_numpy(dtype=np.float32)
