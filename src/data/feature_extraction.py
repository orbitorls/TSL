...
def _build_column_list(feature_level: str = "basic") -> list[str]:
    """Helper to build and cache column names for a given feature level."""
...
def extract_features_from_landmark_df(lm_df: Any, feature_level: str = "basic") -> np.ndarray:
    ...
    # Fast path for empty/missing columns
    if len(available_cols) == 0:
        return np.zeros(feature_dim, dtype=np.float32)

    # ⚡ Bolt: Vectorize pandas mean calculation instead of ~1500 iterative safe_mean calls
