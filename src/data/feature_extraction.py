...
def extract_features_from_landmark_df(lm_df, feature_level='basic'):
    ...
    """
    # Performance Optimization: Cache column names and use Pandas vectorized mean
    # aggregation instead of slow iterative calls to safe_mean.
    expected_cols = _build_column_list(feature_level)
    available_cols = [col for col in expected_cols if col in lm_df.columns]

    if available_cols:
        means_dict = lm_df[available_cols].mean().fillna(0.0).to_dict()
    else:
        means_dict = {}
