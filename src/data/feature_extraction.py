...
def extract_features_from_landmark_df(lm_df: Any, feature_level: str = "basic") -> np.ndarray:
    ...
    result_dict = lm_df[available_cols].mean().fillna(0.0).to_dict()

    features = [result_dict.get(col, 0.0) for col in cols]
...
def extract_sequence_from_landmark_df(
    ...
    col_to_idx = {col: i for i, col in enumerate(col_list)}
    idx_list = [col_to_idx[col] for col in available_cols]

    # Bulk assign missing values handled directly via pandas and bulk placement into array
    seq[:, idx_list] = lm_df[available_cols].fillna(0.0).values
