...
def extract_features_from_landmark_df(lm_df: Any, feature_level: str = "basic") -> np.ndarray:
    ...
    """
    feature_level = validate_feature_level(feature_level)
    col_names = _build_column_list(feature_level)

    # Fast path: Vectorized Pandas computation
    available_cols = lm_df.columns.intersection(col_names)
    ...
    features = [means.get(col, 0.0) for col in col_names]
