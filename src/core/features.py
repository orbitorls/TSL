...
def extract_features(lm_df, feature_level: str = "basic") -> np.ndarray:
    ...
    # ⚡ Bolt Optimization: Use Pandas vectorized mean calculation instead of
    # iterating with safe_mean, yielding a significant speedup for data loading.
    available_cols = [c for c in BASIC_FEATURE_SCHEMA.columns if c in lm_df.columns]
    means = lm_df[available_cols].mean(numeric_only=True).fillna(0.0).to_dict()

    features = [float(means.get(col, 0.0)) for col in BASIC_FEATURE_SCHEMA.columns]
