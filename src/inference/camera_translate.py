...
class ThaiSignTranslator:
    ...
    def _get_cached_font(self, font_size, font_paths=None):
        """Get a cached font to avoid redundant disk I/O in the rendering loop."""
    ...
    def run(self):
        ...
        while True:
            ...
            if self.current_prediction:
                ...
                draw = ImageDraw.Draw(pil_img)
                main_font_paths = ("C:/Windows/Fonts/tahoma.ttf", "C:/Windows/Fonts/phagspa.ttf")
                font = self._get_cached_font(80, main_font_paths)
                conf_font = self._get_cached_font(30, main_font_paths)
            ...
            if len(self.sequence_buffer) >= min_frames:
                ...
                if features is not None and normalized is not None and tensor is not None:
                    ...
                    for index, (label_index, conf) in enumerate(zip(top3_idx, top3_conf)):
                        ...
                        pil_img = PILImage.fromarray(cv2.cvtColor(frame, cv2.COLOR_BGR2RGB))
                        draw = ImageDraw.Draw(pil_img)
                        sidebar_font_paths = ("C:/Windows/Fonts/tahoma.ttf", "C:/Windows/Fonts/phagspa.ttf")
                        font = self._get_cached_font(22, sidebar_font_paths)
