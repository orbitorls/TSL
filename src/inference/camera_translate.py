...
class ThaiSignTranslator:
    ...
    def _get_cached_font(self, font_size, font_paths=None, default_size=None):
        """Retrieve a font from cache, or load and cache it if not found."""
        cache_key = (font_size, tuple(font_paths) if font_paths else None, default_size)
        ...
        if font is None:
            # Fallback to default
            try:
                font = ImageFont.load_default(size=default_size if default_size is not None else font_size)
            except Exception:
    ...
    def _draw_thai_text(self, frame, text, pos, font_size=60, color=(255, 255, 255), bg_color=None):
        ...
        font = self._get_cached_font(font_size, font_paths=font_paths)
    ...
    def run(self):
        ...
        while True:
            ...
            if self.current_prediction:
                ...
                font = self._get_cached_font(80, font_paths=font_paths, default_size=60)
                conf_font = self._get_cached_font(30, font_paths=font_paths, default_size=25)
            ...
            if len(self.sequence_buffer) >= min_frames:
                ...
                if features is not None and normalized is not None and tensor is not None:
                    ...
                    for index, (label_index, conf) in enumerate(zip(top3_idx, top3_conf)):
                        ...
                        font = self._get_cached_font(22, font_paths=font_paths, default_size=18)
